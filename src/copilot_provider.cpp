...
std::vector<std::string> CopilotProvider::get_available_models() const {
    std::vector<std::string> models;
    
    // Get copilot token
    std::string token;
    std::string access_token_unused;
    time_t expires_at_unused;
    if (!const_cast<CopilotProvider*>(this)->load_credentials(token, access_token_unused, expires_at_unused)) {
        std::cerr << "Error: Not authenticated. Please authenticate first.\n";
        return models;
    }
    
    // Query the Copilot models endpoint
    ...
    ResponseData resp;
    std::string url = "https://api.githubcopilot.com/models";
    ...
    try {
        ...
        if (response.contains("data") && response["data"].is_array()) {
            for (const auto& model_obj : response["data"]) {
                ...
                } else if (model_obj.contains("model") && model_obj["model"].is_string()) {
                    models.push_back(model_obj["model"].get<std::string>());
                }
            }
        } else if (response.is_array()) {
            // Handle direct array response
            for (const auto& model_obj : response) {
                if (model_obj.contains("id") && model_obj["id"].is_string()) {
                    models.push_back(model_obj["id"].get<std::string>());
                } else if (model_obj.contains("model") && model_obj["model"].is_string()) {
                    models.push_back(model_obj["model"].get<std::string>());
                } else if (model_obj.is_string()) {
                    models.push_back(model_obj.get<std::string>());
                }
            }
        }
